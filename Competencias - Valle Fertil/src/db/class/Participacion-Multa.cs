...
{
    internal class Participacion_Multa

    {
        ...
        public Participacion_Multa(TimeSpan x) { value = x; }
        public Participacion_Multa(string[] dat) {
            id = int.Parse(dat[0]);
            participacion = ParticipacionService.findOneId(int.Parse(dat[1]));
            string[] tp = dat[2].Split(':');
            this.value = new TimeSpan(0,0,int.Parse(tp[2]),int.Parse(tp[3]),int.Parse(tp[4]));
        }
